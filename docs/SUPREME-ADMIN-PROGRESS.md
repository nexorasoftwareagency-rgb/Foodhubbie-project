...
| 5 | Fill `whatsapp-linking.js` META_APP_ID / META_CONFIG_ID | ✅ | META_APP_ID=1894358871543574 + META_CONFIG_ID=1624840945941910 (set 2026-08-17, matches `docs/WHATSAPP-ONBOARDING-FULL-FLOW.md`) |
...
| G3 | Implement `POST /api/whatsapp/exchange` (Path A, Embedded Signup): app-secret oauth → debug_token → WABA + numbers → save. Fill `META_CONFIG_ID` in whatsapp-linking.js | ✅ | Exchange implemented in server.js (oauth/access_token → debug_token → owned WABAs → phone_numbers → waLinkSuccess). META_CONFIG_ID=1624840945941910 filled 2026-08-17 (was REPLACE_ME). META_APP_SECRET env set on EC2. Path A popup can now launch; Path B works regardless. |
...
- [x] **Bot status "Unknown"** — resolved (C2): PM2 apps renamed to canonical `bot-{bid}-{oid}`, watcher writes live `botStatus`.
  - [x] **META_CONFIG_ID filled** — RESOLVED 2026-08-17: `whatsapp-linking.js:18` now `'1624840945941910'` (Embedded Signup config created in the Meta App Dashboard 2026-08-17, per `docs/WHATSAPP-ONBOARDING-FULL-FLOW.md`). Path A popup can launch. Remaining Path A gaps are account-side: app not yet published (App Review pending business verification).
...
## Chat history tab + WhatsApp coexistence (owner request 2026-08-17)

Owner: *"manage and operate the WhatsApp + Coexistence feature use and directly connect from supreme admin with least taps and clicks… also plan to make a new tab with same WhatsApp real app ui and ux in Admin Dashboard… Build a Chat history tab."*

Deep plan: `docs/PLAN-CHAT-HISTORY-COEXISTENCE.md`. **Feature A (coexistence)** is Meta-side onboarding + our detect/guide; **Feature B (Chat tab)** is the main build (new bot persistence + WhatsApp-UI tab + reply via existing command channel).

| # | Task | Status | Notes |
|---|------|--------|-------|
| H1 | Plan written (this row set) | ✅ | `docs/PLAN-CHAT-HISTORY-COEXISTENCE.md` — data model `chats/{customerId}/{meta,messages}`, two bot hooks (inbound `bot/index.js:1262`, outbound `bot/index.js:1028`), rules (admin-only, PII), WhatsApp-like 2-pane UI, coexistence detect+guide |
| H2 | **B1: bot chat persistence** — `bumpChatMeta` helper + inbound/outbound hooks + `chats` rules | ✅ | `bot/chat-log.js` (`logChatMessage`/`customerIdFromJid`, best-effort `.catch`); inbound hook before opt-out (`bot/index.js` after `[IN]` log); outbound hook fire-and-forget in `sendMessage` patch; `chats` rules admin-only + `.indexOn: lastTs`; `node --test` 10/11 (1 pre-existing whatsapp-send failure, unrelated) |
| H3 | **B2: Admin Chat tab** — `Admin/index.html` nav+div, `Admin/js/features/chat.js`, `Admin/chat.css`, `ui.js` case, PurgeCSS safelist `/^chat-/` | ✅ | WhatsApp 2-pane (thread list: avatar/name/preview/time/unread; conversation: #DCF8C6 out / white in, day separators, ✓✓ ticks, search, composer → `bot/commands` `SEND_GENERIC_MESSAGE`). Built via `tools/build.mjs` — chat.js bundled, all `chat-*` classes survive PurgeCSS. Mobile: panes stack, conversation covers list |
| H3.1 | **Review fixes (chat/coexistence)** | ✅ | ① **Meta replies now logged**: `sendTemplate`/`sendButton` patched too (chat-tab replies on Meta went through `sendTemplate`, not `sendMessage` → thread missed the bot's half + echo vanished). ② Inbound hook now skips `@g.us/@broadcast/@newsletter`. ③ Promo campaigns + rider dispatches tagged `_logChat:false` (no thread pollution). ④ `_initials` bug (full-JID `.slice(-2)` → "ET") fixed. ⑤ Persistent sidebar badge listener survives `cleanupChat`. ⑥ `+91` hardcode removed (uses JID digits). ⑦ Rules `.indexOn: lastTs` moved from `$customerId` → `meta`. ⑧ dead `pending-*` filter deleted. ⑨ shared `_logOutboundChat` guard (admin/group/broadcast/self + `_logChat` opt-out) |
| H4 | **A: coexistence detect+guide** — `webhook-server` reads `msg.origin.type` → `outlet.whatsapp.coexistence`; SupremeAdmin status pill + Enable expandable (3 taps) + "I've enabled it" | ✅ | `webhook-server/index.js`: if `message.origin?.type` is `business_app` → best-effort `.update({mode,lastSeenAt})` on `outlet.whatsapp.coexistence` (never auto-flips OFF; `'update'` origin = message-edited, NOT coexistence — excluded to avoid false-positive pill). `whatsapp-manage.js`: connected view renders `renderCoexistence()` — green `status-pill.online` when detected/enabled, grey `status-pill.unknown` + Enable expandable (WhatsApp Manager link → QR scan → consent, 3 taps) + "I've enabled it" `.update({enabledAt,by})`; trade-off copy. `.update()` both sides (webhook + manual record) so `mode`/`lastSeenAt` and `enabledAt`/`by` coexist — no clobber. No fake API toggle. Rules unchanged (SupremeAdmin write falls through `$businessId.write`). Note added to `docs/WHATSAPP-ONBOARDING-FULL-FLOW.md` §5. **Deploy pending (H5)** |
| H4.1 | **Review fixes (H4 + chat tab, round 2)** | ✅ | ① Coexistence clobber fixed: both writers use `.update()` with merged local state. ② Mobile chat: `#chatBackBtn` added (mobile-only) → removes `chat-conv-open`; `cleanupChat()` also resets it. ③ Badge bound to first-load outlet fixed: single persistent `onValue` listener re-bound on `Outlet.current` change; renders thread list only while tab visible (replaces the two full-subtree listeners). ④ `origin.type==='update'` removed from coexistence trigger (means message-edited, would false-positive ON). ⑤ `chat-log.js` documents last-10-digit country-code collision caveat (matches existing `customers/{cleanPhone}` convention). ⑥ Empty `try/catch{throw}` wrappers dropped |
| H5 | Deploy + verify (B1 → B2 → A) | ✅ | Deployed 2026-08-17: `firebase deploy --only database,hosting:admin,hosting:supreme` (rules released — fixed one RTDB error: `in` operator → explicit `==` comparison in `messages/$msgId` validate); EC2 via SSM base64 (SSH locked): `bot/index.js` (+`chat-log.js`, shipped first — new `require('./chat-log')` would crash a bare index.js restart), `promotions.js`, `rider.js`, `webhook-server/index.js`; md5-verified identical to local (index 69d123, chat-log f0b5a2, promo 276406, rider 69c53b, ws a4e729); `node --check` all clean; `pm2 restart` both apps → online, no crash-loop. **End-to-end smoke:** `logChatMessage` write landed at `chats/9712345678` (meta `unread:1` customer, 0 bot) then removed; hosting serves `/js/features/chat.js` (200) + `/js/features/whatsapp-manage.js` (200). Pre-existing errors in bot err-log (ENOENT `session_data_pizza/creds.json`, `isMetaTransport` at 14:13) predate deploy (err-log mtime 14:13 < 15:27 restart) — not regressions |

## Open questions / blockers (chat/coexistence)
- [ ] **Coexistence cannot be API-enabled** — it's WhatsApp Business App ≥2.24.17 onboarding via WhatsApp Manager (QR + consent). Supreme Admin = status + guidance + intent record only. If owner expects a magic toggle, scope needs to change.
- [ ] **Bot stores no message content today** — Chat tab is gated on H2 (persistence). Until then the tab would be empty.
- [ ] **Reply channel is the 24h window / template constraint** — `SEND_GENERIC_MESSAGE` already template-first with text fallback (`bot_live_update`); Chat composer reuses it, no new send path.
